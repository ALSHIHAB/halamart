        ...
        @if ($product)
            <div class="lg:grid lg:grid-cols-2 lg:gap-x-8 lg:items-start">
                ...
                <div class="px-4 mt-10 sm:px-0 sm:mt-16 lg:mt-0">
                    ...
                    <h1 class="text-xl font-semibold tracking-tight text-gray-900 uppercase sm:text-2xl">
                        {{ $product->getTranslation('name', App::getLocale()) }}
                    </h1>
                    @if ($product->price > 0)
                        <div class="flex items-center mt-2 text-2xl text-gray-900">
                    ...
                    @if ($product->brand_id > 1)
                        <a href="{{ route('brand', $product->brand->getTranslation('slug', App::getLocale())) }}"
                            class="mt-3 text-indigo-600 italic">
                    ...
                    <form class="mt-6">
                        {{-- <x-product.options /> --}}
                        <div class="w-48 mt-5 rtl:text-right">
                            <a target="_blank"
                                href="{{ 'https://wa.me/' . config('global.whatsapp') . '?text=' . route('product.view', $product->getTranslation('slug', App::getLocale())) }}"
                                class="py-1.5 flex flex-row-reverse items-center justify-center px-2 text-green-600 text-base font-medium hover:text-white border rounded-md hover:bg-green-700  hover:border-transparent focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:ring-indigo-500">
                                <svg class="w-6 h-6 fill-current" xmlns="http://www.w3.org/2000/svg"
                                ...
                                <p class="rtl:ml-4 ltr:mr-4">{{ __('Request Product') }}</p>
                            </a>
                        </div>

                        <span class="relative z-0 inline-flex mt-6 rounded-md shadow-sm">
                            @auth
                                @if (auth()->user()->favouriteProduct($product->id))
                                    <button type="button" wire:click="AddToFavourite({{ $product->id }})"
                                        class="relative inline-flex items-center px-4 py-1 text-sm font-medium text-red-700 bg-white border border-gray-300 ltr:rounded-l-md rtl:rounded-r-md hover:bg-gray-50 focus:z-10 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @else
                                    <button type="button" wire:click="AddToFavourite({{ $product->id }})"
                                        class="relative inline-flex items-center px-4 py-1 text-sm font-medium text-gray-700 bg-white border border-gray-300 ltr:rounded-l-md rtl:rounded-r-md hover:bg-gray-50 focus:z-10 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            @if ($product->stock > 0)
                                <button type="button" wire:click="addToCart({{ $product->id }})"
                                    class="relative inline-flex items-center px-4 py-1 text-sm font-medium text-gray-700 bg-white border border-gray-300 ltr:-ml-px rtl:-mr-px hover:bg-gray-50 focus:z-10 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20"
                                        fill="currentColor">
                                        <path
                                            d="M3 1a1 1 0 000 2h1.22l.305 1.222a.997.997 0 00.01.042l1.358 5.43-.893.892C3.74 11.846 4.632 14 6.414 14H15a1 1 0 000-2H6.414l1-1H14a1 1 0 00.894-.553l3-6A1 1 0 0017 3H6.28l-.31-1.243A1 1 0 005 1H3zM16 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM6.5 18a1.5 1.5 0 100-3 1.5 1.5 0 000 3z" />
                                    </svg>
                                    <p class="rtl:mr-4 ltr:ml-4 sm:block">{{ __('Add To Cart') }}</p>
                                </button>

                                <button type="button" wire:click="buyNow({{ $product->id }})"
                                    class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 ltr:-ml-px rtl:-mr-px ltr:rounded-r-md rtl:rounded-l-md hover:bg-gray-50 focus:z-10 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                                    <p class="">{{ __('Buy Now') }}</p>
                                </button>
                            @else
                                <div type="button"
                                    class="relative inline-flex items-center px-4 py-2 text-sm font-medium text-red-700 bg-white border border-gray-300 ltr:-ml-px rtl:-mr-px ltr:rounded-r-md rtl:rounded-l-md hover:bg-gray-50 focus:z-10 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
            ...
            <div class="mt-2 border-t">
                @if (!empty($similars) && count($similars))
                    <div class="mt-10 bg-white">
                        <h2
                            class="text-xl font-semibold text-center text-gray-900 rtl:sm:text-right ltr:sm:text-left sm:text-2xl">
                            {{ __('Similar Products') }}
                        </h2>
                        <div
